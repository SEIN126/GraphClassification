...
class GNNModule(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, args, task='node'):
        ...
        self.convs.append(conv_model(input_dim, hidden_dim[0]))
        assert (args.num_layers >= 1), 'Number of layers is not >=1'
        for l in range(args.num_layers - 1):
            self.convs.append(conv_model(hidden_dim[l], hidden_dim[l+1]))

        # post-message-passing
        self.post_mp = nn.Sequential(
            nn.Linear(hidden_dim[args.num_layers-1], hidden_dim[args.num_layers-1]), nn.Dropout(args.dropout),
            nn.Linear(hidden_dim[args.num_layers-1], output_dim))
...
class GAT(pyg_nn.MessagePassing):
    ...
    def message(self, edge_index_i, x_i, x_j, size_i):
        ...
        e = F.leaky_relu(torch.cat((x_i, x_j), dim=1).mm(self.att),
                         negative_slope=0.2)  # (nodes, 2*f) * (2*f, 1) => (nodes, 1), neagative slope 0.2
