...
class GNNModule(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, CONFIG, task='graph'):
        super(GNNModule, self).__init__()
        
        conv_model = self.build_conv_model(CONFIG['model'])
        
        self.convs = nn.ModuleList()
        self.convs.append(conv_model(input_dim, hidden_dim))
        
        assert (CONFIG['num_layers'] >= 1), 'Number of layers is not >=1'
